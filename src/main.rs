...
impl Game {
    fn score(&mut self, update: &PlayerPoint) {
    ...
    fn adv_score(player: Point, opponent: Point) -> (Point, Point) {
        match (&player, &opponent) {
            (Point::PWin, _) => (Point::PWin, Point::P0),
            (Point::PAdv, _) => (Point::PWin, Point::P0),
            (Point::P40, Point::PAdv) => (Point::P40, Point::P40),
            (Point::P40, Point::P40) => (Point::PAdv, opponent),
            (Point::P40, _) => (Point::PWin, Point::P0),
            (Point::P30, _) => (Point::P40, opponent),
            (Point::P15, _) => (Point::P30, opponent),
            (Point::P0, _) => (Point::P15, opponent)
        }
    }
...
mod tests {
    ...
    fn generic_tennis_test_p1(p1: Point, p2: Point, p1_result: Point, p2_result: Point, is_win: bool) {
        generic_tennis_test(p1, p2, PlayerPoint::P1, p1_result, p2_result, if is_win { GameResult::P1Win } else { GameResult::OnGoing})
    }

    fn generic_tennis_test_p2(p1: Point, p2: Point, p1_result: Point, p2_result: Point, is_win: bool) {
        generic_tennis_test(p2, p1, PlayerPoint::P2, p2_result, p1_result, if is_win { GameResult::P2Win } else { GameResult::OnGoing})
    }

    fn generic_tennis_test(p1: Point, p2: Point, update: PlayerPoint, p1_result: Point, p2_result: Point, game_result: GameResult) {
